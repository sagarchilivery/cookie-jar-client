...
export default function Dashboard() {
  ...
  const handleShowProducts = async (id: string) => {
    try {
    ...
    <Baselayout>
                ...
                arrivals.map((arrival: any) => (
                  ...
                  >
                    ...
                    {arrival.status === "FINISHED" && (
                      <>
                        <p className="text-lg pt-5 text-gray-300">
                          📅{" "}
                          <span className="font-medium">
                            Actual arrival date:
                          </span>{" "}
                        ...
                        <p className="text-lg text-gray-300">
                          🧇{" "}
                          <span className="font-medium">Actual pallets:</span>{" "}
                          {arrival.actual_pallets}
                        </p>
                        <p className="text-lg text-gray-300">
                          📅{" "}
                          <span className="font-medium">Finised on date:</span>{" "}
                          {new Date(arrival.finishDate).toLocaleDateString()}
                        </p>
                      </>
                    )}
                    <p className="text-lg text-gray-300">
            ...
            <>
              ...
              <div className="flex justify-end gap-4 mt-6">
                <button
                  ...
                  className="px-6 py-2 rounded-lg bg-gray-700 text-white hover:bg-gray-600 transition duration-200 cursor-pointer"
                ...
                <ActionButton
                  ...
                  className="px-6 py-2 rounded-lg cursor-pointer bg-green-600 text-white hover:bg-green-500 transition duration-200"
                >
                  Save
                </ActionButton>
              </div>
      ...
      {showProducts && (
        <Modal
          title="Products"
          isOpen={showProducts}
          setIsOpen={setShowProducts}
          parentStyles=" max-w-[1000px]"
          children={
            <div className="pt-4 grid grid-cols-2 max-h-[600px] overflow-y-auto gap-6">
              {products.length > 0 ? (
                products.map((product: any) => (
                  <div
                    key={product.id}
                    className="bg-white/10 p-6 rounded-2xl shadow-xl border border-white/10 backdrop-blur-lg transition-transform transform w-full"
                  >
                    <h2 className="text-2xl font-semibold text-blue-300">
                      {product.name}
                    </h2>
                    {[
                      { emoji: "🏭", key: "SKU", value: "sku" },
                      { emoji: "🗂️", key: "Category", value: "category" },
                      { emoji: "🖍️", key: "Style", value: "style" },
                      { emoji: "🫙", key: "Condition", value: "condition" },
                      { emoji: "🔢", key: "Quantity", value: "quantity" },
                      { emoji: "©️", key: "Brand", value: "name" },
                    ].map((item, i) => (
                      <p className="mt-2 text-gray-300" key={i}>
                        {item.emoji}{" "}
                ...
                <div className="text-center w-full col-span-2 h-[350px] flex items-center justify-center text-xl text-gray-400">
